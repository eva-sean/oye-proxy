...
function handleCall(ws, messageId, action, payload) {
    switch (action) {
        ...
        case 'SetChargingProfile':
            handleSetChargingProfile(ws, messageId, payload);
            break;

        case 'ClearChargingProfile':
            handleClearChargingProfile(ws, messageId, payload);
            break;
...
function handleSetChargingProfile(ws, messageId, payload) {
    const { connectorId, csChargingProfiles } = payload;
    log('INFO', `Setting charging profile for connector ${connectorId}`, csChargingProfiles);
    sendMessage(ws, 3, messageId, { status: 'Accepted' });
}

function handleClearChargingProfile(ws, messageId, payload) {
    log('INFO', 'Clearing charging profiles', payload);
    sendMessage(ws, 3, messageId, { status: 'Accepted' });
}
